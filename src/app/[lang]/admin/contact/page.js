...
export default function AdminContactsPage({ params }) {
    ...
    const { adminKey, status, logout } = useAdminAuth();
    const { contacts, loading, fetchContacts, removeContact } = useAdminContacts(adminKey);
    ...
    useEffect(() => {
        if (status === 'authenticated' && adminKey) {
            fetchContacts();
        }
    }, [status, adminKey, fetchContacts]);
