...
export default function ChatWidget({ t, lang }) {
    ...
    return (
        <>
            ...
            <button
                onClick={toggleChat}
                className={`fixed bottom-6 right-6 md:right-8 z-[999] flex items-center justify-center w-14 h-14 md:w-16 md:h-16 rounded-full shadow-[var(--elevation-3)] transition-all duration-300 hover:scale-105 active:scale-95 hover:shadow-[var(--elevation-4)] animate-float
                    ${chatOpen
                    ? 'translate-y-20 opacity-0 pointer-events-none'
                    : 'translate-y-0 opacity-100 bg-eteq-gradient text-white'
                }
                `}
